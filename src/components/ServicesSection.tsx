...
const ServicesSection = () => {
  ...
  // Service styling configurations
  const serviceStyles = [
    {
        ...
        >
          {siteData.services.map((service, index) => {
            ...
            return (
              ...
              >
                <Card className={`relative p-8 h-full border border-white/20 backdrop-blur-sm bg-gradient-to-br ${style.gradient} hover:border-white/40 transition-all duration-500 group cursor-pointer overflow-hidden shadow-lg hover:shadow-2xl`}>
                  {/* Animated gradient background on hover */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${style.hoverGradient} opacity-0 group-hover:opacity-100 transition-all duration-500`} />
                  
                  {/* Subtle animated border glow */}
                  <div className={`absolute inset-0 rounded-lg bg-gradient-to-r ${style.iconGradient} opacity-0 group-hover:opacity-20 blur-xl transition-all duration-500`} />
                  
                  {/* Content container */}
                  <div className="relative z-10">
                    {/* Icon with gradient background */}
                    <motion.div 
                      className={`inline-flex p-4 rounded-2xl bg-gradient-to-r ${style.iconGradient} text-white mb-6 shadow-lg group-hover:scale-110 group-hover:shadow-2xl transition-all duration-300`}
                      whileHover={{ rotate: 5 }}
                    >
                    ...
                    {service.features && (
                      <ul className="space-y-3 mb-8">
                        {service.features.map((feature, featureIndex) => (
                          ...
                          >
                            <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${style.iconGradient} mr-3 flex-shrink-0`} />
                            {feature}
                          </motion.li>
                        ))}
                      </ul>
                    ...
                    >
                      <Button 
                        className={`w-full bg-gradient-to-r ${style.iconGradient} hover:shadow-lg hover:shadow-${style.glowColor}/25 text-white border-0 transition-all duration-300 group-hover:scale-105`}
                      >
                        Learn More
